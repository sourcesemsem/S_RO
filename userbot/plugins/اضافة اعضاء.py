...
@icssbot.on(admin_cmd(pattern="اضافة ?(.*)"))
@icssbot.on(sudo_cmd(pattern="اضافة ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
CMD_HELP.update(
    {
        "اضافة اعضاء": """**Plugin : **`اضافة اعضاء`

  •  **Syntax : **`.اضافة معرف(s)/او ايدي المستخدم(s)`
