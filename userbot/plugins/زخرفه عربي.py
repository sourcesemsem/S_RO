...
@bot.on(admin_cmd(pattern="زخرف ?(.*)"))
@bot.on(sudo_cmd(pattern="زخرف ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(admin_cmd(pattern="زخرف$", outgoing=True))
@bot.on(sudo_cmd(pattern="زخرف$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    if not reply_message:
        await edit_or_reply(event, "**باضافـة الكلمـة المراد زغرفتها للأمـر .. مثال : .زخرف + كلمـه 💞🧸.**")
        return
    if not reply_message.text:
        await edit_or_reply(event, "**باضافـة الكلمـة المراد زغرفتها للأمـر .. مثال : .زخرف + كلمـه 💞🧸.**")
        return
    chat = "@ZelArbot"
    catevent = await edit_or_reply(event, "**باضافـة الكلمـة المراد زغرفتها للأمـر .. مثال : .زخرف + كلمـه 💞🧸.**")
    async with event.client.conversation(chat) as conv:
        try:
            response = conv.wait_event(
                events.NewMessage(incoming=True, from_users=2076378197)
...
CMD_HELP.update(
    {
        "زخرفه عربي": "`.زخرف` + كلمه او بالـرد ع كلـمه :\
