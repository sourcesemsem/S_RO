...
async def settings(event):
    await event.delete()
    await event.reply(f"⌔∮ انا سزد ثـون مساعد {mention}. الشخصي تستطيع الاتصال بي عن طريق هذا البوت",
                             buttons=[
                                     [Button.inline(
...
async def settings(event):
    await event.edit("** ⌔∮ هنا ستجد رابط زد ثـون ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("🔗 رابط السورس🔗", url="https://github.com/Teamthon/sadpack")],
                         [Button.url("🔗 رابط التنصيب 🔗", url="https://dashboard.heroku.com/new?template=https://github.com/Teamthon/sadpack/")],
                         [Button.url("✨ الدعم ✨",
