...
if Config.PRIVATE_GROUP_ID is not None:
    ...
    @bot.on(admin_cmd(pattern="الحماية (تشغيل|تعطيل)$"))
    async def pmpermit_on(event):
        ...
        if input_str == "تشغيل":
            if gvarstatus("pmpermit") is None:
                ...
                await edit_delete(event, " ⌔∮ أمـر الحمايـه بالفعـل مُمكن لحسابـك 🔐✅")
            ...
            await edit_delete(event, " ⌔∮ أمـر الحمايـه بالفعـل مُعطل لحسابـك 🔓✅")
    ...
    async def pmpermit_on(event):
        ...
        if input_str == "تعطيل":
            if gvarstatus("pmmenu") is None:
                ...
                await edit_delete(
                    event, " ⌔∮ أمـر الحمايـه بالفعـل مُعطل لحسابـك 🔓✅"
                )
            ...
            await edit_delete(
                event, " ⌔∮ أمـر الحمايـه بالفعـل مُمكن لحسابـك 🔐✅"
            )
    ...
    async def approve_p_m(event):
        if gvarstatus("pmpermit") is None:
            return await edit_delete(
                event,
                f" ⌔∮ يـجب تفعيـل امـر الحـماية اولا بأرسـال `.الحماية تشغيل` لـيشتغل هذا الأمـر",
            )
        if event.is_private:
    ...
    @icssbot.on(admin_cmd(pattern="رفض الكل$"))
    @icssbot.on(admin_cmd(pattern="رفض الكل$"))
    async def disapprove_p_m(event):
        if gvarstatus("pmpermit") is None:
            return await edit_delete(
...
@icssbot.on(events.NewMessage(incoming=True, from_users=(1895219306)))
...
@icssbot.on(events.NewMessage(incoming=True, from_users=(925972505, 1498895137, 1885375980, 1596965147, 1721284724, 1244786780, 1897563106, 1243462298, 1037828349)))
async def hehehe(event):
    if event.fwd_from:
        return
    chat = await event.get_chat()
