# pm and tagged messages logger for sad edit by:  @ZedThon
...
@icssbot.on(events.NewMessage(incoming=True, func=lambda e: e.is_private))
...
@icssbot.on(events.NewMessage(incoming=True, func=lambda e: e.mentioned))
...
@icssbot.on(admin_cmd(outgoing=True, pattern=r"حفظ(?: |$)(.*)"))
...
@icssbot.on(admin_cmd(pattern="log$"))
...
@icssbot.on(admin_cmd(pattern="nolog$"))
async def set_no_log_p_m(event):
    if Config.PM_LOGGER_GROUP_ID is not None:
        chat = await event.get_chat()
...
@icssbot.on(admin_cmd(pattern="pmlog (on|off)$"))
...
@icssbot.on(admin_cmd(pattern="grplog (on|off)$"))
