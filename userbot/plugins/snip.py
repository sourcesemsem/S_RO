...
@icssbot.on(admin_cmd(pattern=r"snips (\w*)"))
@icssbot.on(sudo_cmd(pattern=r"snips (\w*)", allow_sudo=True))
...
@icssbot.on(admin_cmd(pattern="snipl$"))
@icssbot.on(sudo_cmd(pattern=r"snipl$", allow_sudo=True))
...
@icssbot.on(admin_cmd(pattern=r"snipd (\S+)"))
@icssbot.on(sudo_cmd(pattern=r"snipd (\S+)", allow_sudo=True))
async def on_snip_delete(event):
    name = event.pattern_match.group(1)
    icssnip = get_note(name)
    if icssnip:
        rm_note(name)
    else:
        return await edit_or_reply(
