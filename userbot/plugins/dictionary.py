# Urban Dictionary for ZedThon edit by @ZedThon
from PyDictionary import PyDictionary

from . import AioHttp


@icssbot.on(admin_cmd(pattern="معن (.*)"))
@icssbot.on(sudo_cmd(pattern="معن (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
CMD_HELP.update(
    ...
    \n\n  •  **Syntax :** `.معن query`\
    \n  •  **Function : **لـ جلب المعنى من القاموس العالمي\
    \n\n  •  **Syntax : **`.معنى`\
    \n  •  **Function : **لـ جلب معنى الكلمة \
    "
    }
)
