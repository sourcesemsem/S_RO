...
@icssbot.on(admin_cmd(pattern="rnup (.*)"))
@icssbot.on(sudo_cmd(pattern="rnup (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    thumb = None
...
@icssbot.on(admin_cmd(pattern="rnupf (.*)"))
@icssbot.on(sudo_cmd(pattern="rnupf (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    thumb = None
