...
@icssbot.on(sudo_cmd(allow_sudo=True, pattern="????? (.*)"))
@errors_handler
...
@icssbot.on(admin_cmd(outgoing=True, pattern="????? (en|de) ?(.*)"))
@icssbot.on(sudo_cmd(allow_sudo=True, pattern="????? (en|de) ?(.*)"))
@errors_handler
...
CMD_HELP.update(
    {
        "?????": "**??? ???????? : **`?????`\
        \n\n**??? ?????? ? **`.????? en toencode-text or .????? de encoded-text`\
        \n**?????? ?? **__???? ?? ????? base64 ??????? ??????? ?? ?? ????? ???????__\
        \n\n**??? ?????? ? **`.????? + ??`\
        \n**?????? ?? **__???? ?? md5 ? sha1 ? sha256 ? sha512 ??????? ??? ??????? ?? ??? txt__"
    }
)
