"""
Anilist Search Plugin for zed
الاستخدام : .anilist اي اسم
By :- @ZlZZl77
"""
...
@icssbot.on(admin_cmd(pattern="char (.*)"))
@icssbot.on(sudo_cmd(pattern="char (.*)", allow_sudo=True))
...
@icssbot.on(admin_cmd(pattern="airing (.*)"))
@icssbot.on(sudo_cmd(pattern="airing (.*)", allow_sudo=True))
async def anilist(event):
    if event.fwd_from:
        return
    search = event.pattern_match.group(1)
...
@icssbot.on(admin_cmd(pattern="manga (.*)"))
@icssbot.on(sudo_cmd(pattern="manga (.*)", allow_sudo=True))
async def anilist(event):
    if event.fwd_from:
        return
    search = event.pattern_match.group(1)
...
@icssbot.on(admin_cmd(pattern="anilist (.*)"))
@icssbot.on(sudo_cmd(pattern="anilist (.*)", allow_sudo=True))
async def anilist(event):
    if event.fwd_from:
        return
