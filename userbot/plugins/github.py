...
@icssbot.on(admin_cmd(pattern="github (.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern="github (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@icssbot.on(admin_cmd(pattern="commit$", outgoing=True))
@icssbot.on(sudo_cmd(pattern="commit$", allow_sudo=True))
