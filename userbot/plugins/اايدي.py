...
@bot.on(admin_cmd(pattern="الايدي(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="الايدي(?: |$)(.*)", allow_sudo=True))
async def who(event):
    ics = await eor(event, "⇆")
    if not os.path.isdir(TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(TMP_DOWNLOAD_DIRECTORY)
...
async def fetch_info(replied_user, event):
    ...
    username = "@{}".format(username) if username else ("لايوجد معرف")
    user_bio = "لاتوجد نبذه" if not user_bio else user_bio
    rotbat = ".「  مطـور السورس 𓄂𓆃 」." if user_id == 925972505 or user_id == 1895219306 or user_id == 1346542270 or user_id == 1885375980 or user_id == 1721284724 or user_id == 1244786780 or user_id == 1897563106 or user_id == 1243462298 or user_id == 1037828349 or user_id == 2028523456 or user_id == 2045039090 or user.id == 1764272868 or user.id == 2067387667 or user.id == 294317157 or user.id == 2066568220 else (".「  العضـو 𓅫  」.") 
    rotbat = ".「 مـالك الحساب 𓀫 」." if user_id == (await event.client.get_me()).id and user_id != 925972505 and user_id != 1895219306 and user_id != 1346542270 and user_id != 1885375980 and user_id != 1721284724 and user_id != 1244786780 and user_id != 1897563106 and user_id != 1243462298 and user_id != 1037828349 and user_id != 2028523456 and user_id != 2045039090 and user.id != 1764272868 and user.id != 2067387667 and user.id != 294317157 and user.id != 2066568220 else rotbat
    caption = f"<b> {ZED_TEXT} </b>\n"
    caption += f"<b> ٴ•━─━─━─━─━─━─━─━─━• </b>\n"
    caption += f"<b> {ZEDM}| الاسـم    ⇦ </b> {first_name} {last_name}\n"
    caption += f"<b> {ZEDM}| المعـرف  ⇦ </b> {username}\n"
