...
@icssbot.on(admin_cmd(pattern="report$"))
@icssbot.on(sudo_cmd(pattern="report$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@icssbot.on(admin_cmd(pattern="men (.*)"))
@icssbot.on(sudo_cmd(pattern="men (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
