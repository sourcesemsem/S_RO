...
QQ070VP_cmd = (
    ...
    "\n𓆩 [ 𖠧 ๏͈͈͈͈͈͈͈𝐑𝐀𝐈𝐈𝐒 𖤓̟̟̟̟̟̟̥̥̥̥̟͜͡✪](t.me/QQ070) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await Ralls.edit("**✾╎تم تغييـر {} بنجـاح ☑️**\n**✾╎المتغير : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
            await Ralls.edit("**✾╎تم اضافـة {} بنجـاح ☑️** \n**✾╎المضاف اليه :**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Ralls.edit("**✾╎تم تغيـر المنطقـة الزمنيـه**\n **✾╎المتغير : دولـة مصـر 🇪🇬**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        else:
            await Ralls.edit("**✾╎تم اعـادة تغيـر المنطقـة الزمنيـه**\n **✾╎المتغير : دولـة مصـر 🇪🇬**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        heroku_var[variable] = vinfo
        ...
        if input_str:
            return await Ralls.edit("**✾╎عـذراً .. لايوجـد هنالك فـار بإسـم {} ؟!..**".format(input_str))
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await Ralls.edit("**✾╎تم حـذف {} بنجـاح ☑️**\n**✾╎المتغيـر المحـذوف : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def variable(event):
    ...
    if input_str == "رمز الوقتي" or input_str == "رمز الاسم الوقتي":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Ralls.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس : @RallsThon**".format(input_str, heroku_var[variable]))
        else:
            await Ralls.edit("**✾╎ الفـار {} غيـر موجـود ❌** \n**✾╎المتغيـر :**\n `{}` \n**✾╎قنـاة السـورس : @RallsThon**".format(input_str, heroku_var[variable]))
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Ralls.edit("**✾╎المطـور {} موجـود 🧑🏻‍💻☑️**\n**✾╎ايدي المطـور : ↶**\n `{}` \n**✾╎قنـاة السـورس : @RallsThon**".format(input_str, heroku_var[variable]))
        else:
            await Ralls.edit("**✾╎ المطـور {} غيـر موجـود 🧑🏻‍💻❌** \n**✾╎المتغيـر :**\n `{}` \n**✾╎قنـاة السـورس : @RallsThon**".format(input_str, heroku_var[variable]))
...
async def variable(event):
    ...
    vmsr = "Africa/Cairo"
    heroku_var = app.config()
    Ralls = await edit_or_reply(event, "**✾╎جـاري أعـداد المنطقـه الزمنيـه لـ ريسثــون 🌐...**")
    # All Rights Reserved for "Rallsthon - UserBot" "زلـزال الهيبـه"
    if input_str == "العراق" or input_str == "عراق":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Ralls.edit("**✾╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المتغير : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
            await Ralls.edit("**✾╎تم اضـافـة المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المضـاف اليـه : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
...
async def QQ070ll(event):
    ...
    await asyncio.sleep(1.5)
    if variable in heroku_var:
        await Ralls.edit("**✾╎تـم رفـع**  [الشخـص🧑🏻‍💻](tg://user?id={})  **مطـور .. بنجـاح ☑️**\n**✾╎بصـلاحيات تحـكم كـامل بالاوامـر 🦾**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(user_id))
    else:
        await Ralls.edit("**✾╎تـم إعـادة رفـع**  [الشخـص🧑🏻‍💻](tg://user?id={})  **مطـور .. بنجـاح ☑️**\n**✾╎بصـلاحيات تحـكم كـامل بالاوامـر 🦾**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(user_id))
...
async def QQ070ll(event):
    ...
    if previous_message:
	    ...
	    await asyncio.sleep(1.5)
	    if variable not in heroku_var:
	        return await Ralls.edit(f"**✾╎عـذراً .. فـار {input_str} غير موجود اصـلاً...**")
	
	    await Ralls.edit("**✾╎تـم تنزيـل**  [الشخـص🧑🏻‍💻](tg://user?id={})  **مطـور .. بنجـاح ☑️**\n**✾╎لـم يعد لديـه تحـكم بأوامـر البـوت 🥲**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(user_id))
	    del heroku_var[variable]
...
async def _(tosh):
    ...
    heroku_var = app.config()
    Ralls = await eor(tosh, "**✾╎جـاري اضـافة فـار الصـورة الـى بـوتك ...**")
    ...
    if tosh.reply_to_msg_id:
        ...
        if input_str in ["الحماية", "الحمايه"]:
            ...
            try:
                ...
                await Ralls.edit("**✾╎تم تغييـر صـورة {} .. بنجـاح ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت ريس ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
