# pm and tagged messages logger for zed edit by:  @ZZZZl1l
...
async def log(log_text):
    if BOTLOG:
        if log_text.reply_to_msg_id:
            ...
            user = f"#LOG / ايدي الدردشـه: {log_text.chat_id}\n\n"
        ...
        await log_text.edit("**⌔∮تتطلب هذه الميزة تمكين التسجيل! ...**")
    await asyncio.sleep(2)
    await log_text.delete()


@bot.on(admin_cmd(pattern="لوج$"))
...
@bot.on(admin_cmd(pattern="نولوج$"))
async def set_no_log_p_m(event):
    if Config.PM_LOGGER_GROUP_ID is not None:
        chat = await event.get_chat()
...
@bot.on(admin_cmd(pattern="تسجيل (تفعيل|تعطيل)$"))
async def set_pmlog(event):
    ...
    if PMLOG:
        if h_type:
            await event.edit("**⌔∮تسـجيل الرسائـل ممكّن بالفعـل ✅...**")
        else:
            addgvar("PMLOG", h_type)
            await event.edit("**⌔∮تم تعطيـل تسـجيل الرسائـل ✅...**")
    else:
        if h_type:
            addgvar("PMLOG", h_type)
            await event.edit("**⌔∮تم تفعيـل تسـجيل الرسائـل ✅...**")
        else:
            await event.edit("**⌔∮تسـجيل الرسائـل معطـل بالفعـل ✅...**")


@bot.on(admin_cmd(pattern="كروب (تفعيل|تعطيل)$"))
async def set_grplog(event):
    ...
    if GRPLOG:
        if h_type:
            await event.edit("**⌔∮تسـجيل المجموعـه ممكّن بالفعـل ✅...**")
        else:
            addgvar("GRPLOG", h_type)
            await event.edit("**⌔∮تم تعطيـل تسـجيل المجموعـه ✅...**")
    else:
        if h_type:
            addgvar("GRPLOG", h_type)
            await event.edit("**⌔∮تم تفعيـل تسـجيل المجموعـه ✅...**")
        else:
            await event.edit("**⌔∮تسـجيل المجموعـه معطـل بالفعـل ✅...**")


CMD_HELP.update(
    {
        "الحافظه": "**اسم الاضافـه : **`الحافظه`\
        \n\n  **╮•❐ الامـر ⦂ **`.حفظ`\
        \n•  **الشـرح •• **__يحفظ الرسائـل المحـدده في مجموعـتك الخاصـه ( كروب حافظة بوتك ) .__\
        \n\n  **╮•❐ الامـر ⦂ **`.لوج`\
        \n•  **الشـرح •• **__بشكل افتراضي ، سيتم تسجيل جميع رسائل الدردشة الخاصة إذا كنت تستخدم الامر ( .نولوج ) وتريد تسجيل الدخول مرة أخرى ، فأنت بحاجة إلى استخدام هذا الامر ( .لوج ) ...__\
        \n\n  **╮•❐ الامـر ⦂ **`.نولوج`\
        \n•  **الشـرح •• **__يوقف التسجيل من الدردشة الخاصة أو المجموعة التي استخدمتها__\
        \n\n  **╮•❐ الامـر ⦂ **`.تسجيل تفعيل/تعطيل`\
        \n•  **الشـرح •• **__لتشغيل وإيقاف تشغيل تسجيـل الرسائل الشخصيـه ...__\
        \n\n  **╮•❐ الامـر ⦂ **`.كروب تفعيل/تعطيل`\
