...
@bot.on(icss_cmd(pattern="(بحث|اغنيه)($| (.*))"))
@bot.on(sudo_cmd(pattern="(بحث|اغنيه)($| (.*))", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    icse = await eor(event, "**╮ جـارِ البحث ؏ـن الاغنيـٓه... 🎧♥️╰**")
    ...
    await icse.edit("**╮ ❐ جـارِ تحميـل الاغنيـٓه انتظـر قليلاً  ▬▭... 𓅫╰**")
...
@bot.on(icss_cmd(pattern="فيديو( (.*)|$)"))
@bot.on(sudo_cmd(pattern="فيديو( (.*)|$)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    if event.pattern_match.group(1):
        ...
        return
    ics = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    icse = await eor(event, "**╮ جـارِ البحث ؏ـن الفيديـو 🎬♥️╰**")
    video_link = await yt_search(str(query))
    if not url(video_link):
        return await icse.edit(
            f"**⌔╎آسف** {mention}\n **-لا يمكنني العثور على أي #فيديو او #صوت كهذا \n - `{query}` ⇲"
...
@bot.on(admin_cmd(pattern="اغنية (.*)"))
@bot.on(sudo_cmd(pattern="اغنية (.*)", allow_sudo=True))
