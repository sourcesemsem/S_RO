# THIS FILE PART FROM - SOURCE ZED
# © @zlzzl77 @zlzzl77
...
@icssbot.on(admin_cmd(pattern="cascheck$"))
@icssbot.on(sudo_cmd(pattern="cascheck$", allow_sudo=True))
...
@icssbot.on(admin_cmd(pattern="spamcheck$"))
@icssbot.on(sudo_cmd(pattern="spamcheck$", allow_sudo=True))
async def caschecker(cas):
    text = ""
    chat = cas.chat_id
    catevent = await edit_or_reply(
