...
@icssbot.on(admin_cmd(outgoing=True, pattern=r"wiki (.*)"))
@icssbot.on(sudo_cmd(allow_sudo=True, pattern=r"wiki (.*)"))
...
@icssbot.on(admin_cmd(pattern="imdb (.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern="imdb (.*)", allow_sudo=True))
async def imdb(e):
    icsevent = await edit_or_reply(e, "`searching........")
    try:
        ...
        await icsevent.edit("Plox enter **Valid movie name** kthx")
