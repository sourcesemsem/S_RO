...
@bot.on(admin_cmd(pattern="انمي ?(.*)"))
@bot.on(sudo_cmd(pattern="انمي ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    if not reply_to_id:
        return await edit_or_reply(
            event, "**╮ . اكثـر مـن 1000 افتـارات انمـي شبـاب ممطـروقـه.. ارسـل .انمي ولد 𓅫╰**"
        )
    chat = "@ZelTrbot"
...
async def _(ZEDDevent):
    ...
    await edit_or_reply(ZEDDevent, f"𓆰 𝙎𝙊𝙐𝙍𝘾𝞝 𝙕𝞝𝘿 - 𝙑𝙄𝙎𝘼_𝘾𝘼𝙍𝘿  💳𓆪\n\n\n__**👤 الاسـم :- **__\n ٴ──┄──┄──┄──┄──\n`{ZEDDname}`\n\n__**🏡 العنـوان :- **__\n ٴ──┄──┄──┄──┄──\n`{ZEDDadre}`\n\n__**💸 الفيـزا :- **__\n ٴ──┄──┄──┄──┄──\n`{ZEDDcard}`\n\n◟𝙎𝙤𝙪𝙧𝙘𝙚 𝙕𝙚𝙙𝙏𝙝𝙤𝙣 ➧ @ZedThon◞ ")
...
CMD_HELP.update(
    ...
    \n\n**╮•❐ الامـر ⦂ **`.انمي ولد`  \
    \n**الشـرح •• **اكثـر مـن 1000 افتـارات انمـي شبـاب ممطـروقـه .. ارسـل .انمي ولد"
    }
)
