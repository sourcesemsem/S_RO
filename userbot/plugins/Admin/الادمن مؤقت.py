...
@icssbot.on(admin_cmd(pattern=r"اكتم(?: |$)(.*)"))
@icssbot.on(sudo_cmd(pattern=r"اكتم(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def tmuter(kimo):
    ...
    if not admin and not creator:
        ...
        return
    icse = await edit_or_reply(kimo, "╮ ❐ جـاري الڪتم 𓅫╰")
    ...
    if user.id == self_user.id:
        await icse.edit(f"**⪼ لا استطيـع كتم نفسـي 𓆰،**")
        return
    try:
        await icse.client(
            EditBannedRequest(
...
@icssbot.on(admin_cmd(pattern="احظر(?: |$)(.*)"))
@icssbot.on(sudo_cmd(pattern="احظر(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def ban(kimo):
    chat = await kimo.get_chat()
    admin = chat.admin_rights
    ...
    if not admin and not creator:
        await edit_or_reply(kimo, NO_ADMIN)
        return
    icse = await edit_or_reply(kimo, "╮ ❐ جـاري الحـظࢪ ❏╰")
    user, reason = await get_user_from_event(kimo, icse)
    if not user:
        return
    if reason:
    ...
    if not itime:
        ...
        return
    if user.id == self_user.id:
        await icse.edit(f"**⪼ لا استطيـع حظر نفسـي 𓆰،**")
        return
