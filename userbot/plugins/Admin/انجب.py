...
@icssbot.on(admin_cmd(outgoing=True, pattern=r"انجب ?(\d+)?"))
@icssbot.on(sudo_cmd(pattern=r"انجب ?(\d+)?", allow_sudo=True))
async def startgmute(event):
    ...
    if user.id == 925972505:
        await event.edit("**╮ ❐  لا يمڪنني حظر مطـوري  ❏╰**")
        return
...
@icssbot.on(
    icss_cmd(outgoing=True, pattern=r"الغاء انجب ?(\d+)?"
    )
)
@icssbot.on(sudo_cmd(pattern=r"الغاء انجب ?(\d+)?", allow_sudo=True))
