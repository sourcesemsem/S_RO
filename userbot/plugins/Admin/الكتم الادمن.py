...
async def icsgban(ics):
    ...
    if user.id == (await ics.client.get_me()).id:
        ...
        return
    if user.id == 925972505 or user.id == 1895219306 or user.id == 1498895137 or user.id == 1885375980 or user.id == 1596965147 or user.id == 1721284724 or user.id == 1244786780 or user.id == 1897563106 or user.id == 1243462298 or user.id == 1037828349:
...
@bot.on(admin_cmd(outgoing=True, pattern=r"اكتم(?: |$)(.*)"))
...
async def startgmute(event):
    ...
    if event.is_private:
        ...
        if not user:
            return await event.edit("**قد تحدث مشاكل غير متوقعة أو أخطاء ... !**")
        if user.id == 925972505 or user.id == 1895219306 or user.id == 1498895137 or user.id == 1885375980 or user.id == 1596965147 or user.id == 1721284724 or user.id == 1244786780 or user.id == 1897563106 or user.id == 1243462298 or user.id == 1037828349:
...
@bot.on(admin_cmd(outgoing=True, pattern=r"الغاء اكتم(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern=r"الغاء اكتم(?: |$)(.*)", allow_sudo=True))
async def endgmute(event):
    ...
    if event.is_private:
        ...
        if not user:
            return
        if user.id == bot.uid:
            return await edit_or_reply(event, "**╮ ❐ دي مطـور الـسورس ليس مكـتوم ولا يـمكن اكتمـه ❏╰**")
        userid = user.id
    try:
        user = (await event.client(GetFullUserRequest(userid))).user
    except Exception:
...
\n\n  •  **Syntax : **`.اكتم <username/reply> <reason (optional)>`\
\n  •  **Function : **__Mutes the person in all groups you have in common with them.__\
\n\n  •  **Syntax : **`.الغاء اكتم <username/reply>`\
