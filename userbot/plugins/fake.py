...
@icssbot.on(admin_cmd(pattern="scam(?: |$)(.*)"))
@icssbot.on(sudo_cmd(pattern="scam(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    options = [
...
@icssbot.on(admin_cmd(pattern="prankpromote ?(.*)"))
@icssbot.on(sudo_cmd(pattern="prankpromote ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    datetime.now()
...
@icssbot.on(admin_cmd(pattern=f"padmin$", outgoing=True))
@icssbot.on(sudo_cmd(pattern="padmin$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
