...
@icssbot.on(admin_cmd(pattern="لصوره$"))
...
@icssbot.on(admin_cmd(pattern="لملصق$"))
@icssbot.on(sudo_cmd(pattern="لملصق$", allow_sudo=True))
async def _(cat):
    if cat.fwd_from:
        return
    reply_to_id = cat.message.id
...
@icssbot.on(admin_cmd(pattern="لمتحرك(?: |$)(.*)"))
@icssbot.on(sudo_cmd(pattern="لمتحرك(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@icssbot.on(admin_cmd(pattern="حول ?(.*)"))
@icssbot.on(sudo_cmd(pattern="حول ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
#ZedThon 
@icss.on(icss_cmd(pattern="متحرك ?(.*)"))
@icssbot.on(sudo_cmd(pattern="متحرك ?(.*)", allow_sudo=True))
async def gifs(ult):
...
CMD_HELP.update(
    {
        "التحويلات": "**Plugin : **`التحويلات`\
    \n\n**Syntax : **`.لصوره` بالرد ؏ الملـصق\
    \n**Usage :**تحويل ملـصق لــ صـوره\
    \n\n**Syntax : **`.لملصق` reply to image\
    \n**Usage :**تحويل صوره لــ ملصق\
    \n\n**Syntax :** `.ftoi` reply to image file\
    \n**Usage :** Converts Given image file to straemable form\
    \n\n**Syntax :** `.لمتحرك` بالـرد ؏ ملصق متحـرك\
    \n**Usage :** تحويل ملصق متحرك لــ متحـركة\
    \n\n**Syntax :** `.متحرك` + اسم المتحرك\
    \n**Usage :** للبحـث عن متحركات بالاسم سواء عربي او انكلش\
    \n\n**Syntax :** `.ttf file name` reply to text message\
    \n**Usage :** Converts Given text message to required file(given file name)\
    \n\n**Syntax :**`.حول بصمه` or `.حول صوت` بالـرد على فيديو :\
